            ...
            <div class="panel">
                ...
                <div class="search-box">
                    <form action="${pageContext.request.contextPath}/admin/orders" method="GET">
                        <label for="txtOrderID" style="font-weight: bold; font-size: 1.1em;">Nhập mã đơn hàng sinh viên cung cấp: </label>

                        <input type="number" id="txtOrderID" name="orderID" required placeholder="Ví dụ: 15">

                        <button type="submit" class="btn btn-approve" style="padding: 10px 20px;">Tra cứu / Kiểm tra</button>
                    </form>
                </div>
                ...
                <c:if test="${not empty searchResult}">
                    <div class="details-box">
                        ...
                        <p>Tổng tiền phải thu: <strong style="color:red; font-size: 1.2em;">${searchResult.totalAmount} VNĐ</strong></p>
                        ...
                        <c:if test="${searchResult.status eq 'Sẵn sàng' or searchResult.status eq 'Hàng chờ' or searchResult.status eq 'Pending'}">
                            <div style="margin-top: 15px; display: flex; gap: 10px;">
                                <form method="POST" action="${pageContext.request.contextPath}/admin/orders" onsubmit="return confirm('Xác nhận ĐÃ THU TIỀN và giao sách cho sinh viên? (Hành động này sẽ trừ số lượng sách trong kho)');">
                                    <input type="hidden" name="action" value="complete">
                                    <input type="hidden" name="orderID" value="${searchResult.orderID}">
                                    <button type="submit" class="btn btn-approve" style="padding: 10px 20px;">Đã thu tiền & Hoàn thành giao sách</button>
                                </form>

                                <form method="POST" action="${pageContext.request.contextPath}/admin/orders" onsubmit="return confirm('Bạn có chắc chắn muốn HỦY đơn này?');">
                                    <input type="hidden" name="action" value="cancel">
                                    <input type="hidden" name="orderID" value="${searchResult.orderID}">
                                    <button type="submit" class="btn btn-reject" style="padding: 10px 20px;">Hủy bỏ đơn</button>
                                </form>
                            </div>
                        </c:if>
                    </div>
                    ...
                    <tbody>
                        <c:forEach var="order" items="${allOrders}">
                            <tr>
                                ...
                                <td>
                                    <span style="font-weight: bold; color:
                                          ${order.status eq 'Đã giao' ? 'green' : 
                                            (order.status eq 'Đã hủy' ? 'red' : 
                                    ...
                                    <td>
                                        <c:if test="${order.status ne 'Đã giao' and order.status ne 'Đã hủy'}">
                                            <a href="${pageContext.request.contextPath}/admin/orders?orderID=${order.orderID}" class="btn btn-approve" style="text-decoration: none; padding: 5px 10px; display: inline-block;">Xử lý</a>
                                        </c:if>
                                    </td>
                                </tr>
                            </c:forEach>
                            <c:if test="${empty allOrders}">
                                <tr>
                                    <td colspan="8" class="empty-row" style="text-align: center;">Chưa có đơn hàng nào.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
