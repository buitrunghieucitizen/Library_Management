...
<body>
    ...
    <div class="container">
        ...
        <a class="btn btn-primary" href="${pageContext.request.contextPath}/students?action=create" style="margin-bottom:15px;display:inline-block;">+ Thêm sinh viên</a>
        <table>
            <thead><tr><th>ID</th><th>Tên</th><th>Email</th><th>SĐT</th><th>Hành động</th></tr></thead>
            <tbody>
                <c:forEach var="s" items="${students}">
                    <tr>
                        <td>${s.studentID}</td><td>${s.studentName}</td><td>${s.email}</td><td>${s.phone}</td>
                        <td class="actions">
                            <a class="btn btn-warning" href="${pageContext.request.contextPath}/students?action=edit&id=${s.studentID}">Sửa</a>
                            <a class="btn btn-danger" href="${pageContext.request.contextPath}/students?action=delete&id=${s.studentID}" onclick="return confirm('Xóa?')">Xóa</a>
                        </td>
                    </tr>
                </c:forEach>
                <c:if test="${empty students}"><tr><td colspan="5" style="text-align:center;color:#999;padding:30px;">Chưa có sinh viên.</td></tr></c:if>
            </tbody>
        </table>
    </div>
</body>
