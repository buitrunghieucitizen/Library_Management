...
public class DAOStaff {
    ...
    public Staff login(String username, String password) throws SQLException {
        ...
        String sql = "SELECT StaffID, StaffName, Username, Email, Password FROM Staff WHERE Username = ? AND Password = ?";
    ...
    public Staff getByUsername(String username) throws SQLException {
        ...
        String sql = "SELECT StaffID, StaffName, Username, Email, Password FROM Staff WHERE Username = ?";
        Connection con = DBConnection.getConnection();
        if (con == null) {
    ...
    public Staff getByEmail(String email) throws SQLException {
        ...
        String sql = "SELECT StaffID, StaffName, Username, Email, Password FROM Staff WHERE Email = ?";
        Connection con = DBConnection.getConnection();
        if (con == null) {
            throw new SQLException("Cannot connect to database!");
        }
    ...
    public Staff loginByGoogle(GoogleAccount googleAccount) throws SQLException {
        ...
        Staff existing = getByEmail(normalizedEmail);
        if (existing == null) {
            existing = getByUsername(username);
        }
        if (existing != null) {
            if (isBlank(existing.getEmail())) {
        ...
        Staff created = new Staff(displayName, username, normalizedEmail, UUID.randomUUID().toString());
        try {
            insert(created);
        ...
        Staff createdOrExisting = getByEmail(normalizedEmail);
    ...
    public List<Staff> getAll() throws SQLException {
        String sql = "SELECT StaffID, StaffName, Username, Email, Password FROM Staff ORDER BY StaffID DESC";
    ...
    public Staff getById(int id) throws SQLException {
        String sql = "SELECT StaffID, StaffName, Username, Email, Password FROM Staff WHERE StaffID = ?";
        Connection con = DBConnection.getConnection();
        if (con == null) throw new SQLException("Cannot connect to database!");
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, id);
    ...
    public int insert(Staff s) throws SQLException {
        String sql = "INSERT INTO Staff(StaffName, Username, Email, Password) VALUES(?,?,?,?)";
        Connection con = DBConnection.getConnection();
        if (con == null) throw new SQLException("Cannot connect to database!");
    ...
    public int update(Staff s) throws SQLException {
        String sql = "UPDATE Staff SET StaffName=?, Username=?, Email=?, Password=? WHERE StaffID=?";
        Connection con = DBConnection.getConnection();
        if (con == null) throw new SQLException("Cannot connect to database!");
        try (PreparedStatement ps = con.prepareStatement(sql)) {
            String normalizedUsername = normalizeUsername(s.getUsername());
