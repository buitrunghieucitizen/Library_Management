...
<c:set var="pageTitle" value="Quản lý tệp sách" />
    ...
    <div class="container">
        <div class="panel">
            ...
            <c:if test="${not empty param.msg}"><div class="msg mb-3">${param.msg}</div></c:if>
            <c:if test="${not empty param.error}"><div class="error mb-3">${param.error}</div></c:if>
            <div class="note">Tổng bản ghi: ${totalItems}</div>

            <table>
                <thead>
                    <tr>
                        <th>Mã</th><th>Sách</th><th>Nhân viên</th><th>Tệp</th><th>Liên kết</th><th>Loại</th><th>Kích thước</th><th>Ngày tải</th><th>Kích hoạt</th><th>Hành động</th>
                    </tr>
                </thead>
                <tbody>
                    <c:forEach var="row" items="${bookFiles}">
                        <tr>
                            <td>${row.bookFileID}</td><td>${row.bookName}</td><td>${row.staffName}</td>
                            <td>${row.fileName}</td>
                            <td><a href="${row.fileUrl}" target="_blank">${row.fileUrl}</a></td>
                            <td>${row.fileType}</td><td>${row.fileSize}</td><td>${row.uploadAt}</td>
                    ...
                    <c:if test="${empty bookFiles}">
                        <tr><td colspan="10" class="empty-row">Chưa có tệp sách nào.</td></tr>
                    </c:if>
                </tbody>
            </table>
            ...
            <c:if test="${totalPages > 1}">
                <div class="pagination">
                    <c:if test="${currentPage > 1}">
                        <c:url var="prevUrl" value="/admin/bookfiles"><c:param name="action" value="list"/><c:param name="page" value="${currentPage - 1}"/></c:url>
                        <a class="page-link" href="${prevUrl}">Trang trước</a>
                    </c:if>
                    <c:forEach begin="1" end="${totalPages}" var="p">
                        <c:url var="pageUrl" value="/admin/bookfiles"><c:param name="action" value="list"/><c:param name="page" value="${p}"/></c:url>
                        <a class="page-link ${p eq currentPage ? 'active' : ''}" href="${pageUrl}">${p}</a>
                    </c:forEach>
                    <c:if test="${currentPage < totalPages}">
                        <c:url var="nextUrl" value="/admin/bookfiles"><c:param name="action" value="list"/><c:param name="page" value="${currentPage + 1}"/></c:url>
