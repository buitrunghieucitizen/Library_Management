        ...
        <main class="content student-content content-wide">
            ...
            <section class="student-highlight-grid">
                <article class="student-highlight-card good">
                    ...
                    <p>
                        <c:choose>
                            <c:when test="${readyBuyCount gt 0}">
                                Các mục này đã đủ giá và tồn kho để đưa vào checkout ngay.
                            </c:when>
                            <c:otherwise>
                                Hiện chưa có mục nào trong danh sách cần mua đủ điều kiện để checkout.
                            </c:otherwise>
                        </c:choose>
                    </p>
                </article>
                <article class="student-highlight-card neutral">
                    ...
                    <p>
                        <c:choose>
                            <c:when test="${safePurchaseTotalItems gt 0}">
                                Có thể tra cứu lại lịch sử đơn đã hoàn tất ở khu vực đã mua.
                            </c:when>
                            <c:otherwise>
                                Bạn chưa có đơn mua nào được duyệt trong hệ thống.
                            </c:otherwise>
                        </c:choose>
                    </p>
                </article>
                    ...
                    <p>
                        <c:choose>
                            <c:when test="${overdueCount gt 0}">
                                Có phiếu quá hạn. Nên gửi yêu cầu trả để thư viện xử lý sớm.
                            </c:when>
                            <c:otherwise>
                                Chưa có phiếu nào quá hạn. Hiện bạn đang theo dõi ${activeBorrowCount} phiếu mở.
                            </c:otherwise>
                        </c:choose>
                    </p>
                </article>
                    ...
                    <table class="compact-table">
                        ...
                        <tbody>
                            <c:forEach var="price" items="${bookPrices}">
                                <tr>
                                    ...
                                    <td>
                                        <c:choose>
                                            <c:when test="${price.amount > 0}">
                                                <span class="status-ok">Có thể mua</span>
                                            </c:when>
                                            <c:otherwise>
                                                <span class="status-bad">Chờ giá</span>
                                            </c:otherwise>
                                        </c:choose>
                                    </td>
                            ...
                            <c:if test="${empty bookPrices}">
                                <tr>
                                    <td colspan="7" class="empty">Chưa có bảng giá sách.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
                    ...
                    <table>
                        ...
                        <tbody>
                            ...
                            <c:if test="${empty availableBooks}">
                                <tr>
                                    <td colspan="4" class="empty">Không có sách phù hợp với bộ lọc hiện tại.</td>
                                </tr>
                            </c:if>
                        </tbody>
                    </table>
                ...
                <section class="card table-card" id="buy-list-panel">
                    <div class="section-header-inline student-section-head">
                        ...
                        <div class="student-head-actions">
                            ...
                            <c:if test="${blockedBuyCount gt 0}">
                                <span class="student-chip warning">${blockedBuyCount} cần kiểm tra</span>
                            </c:if>
                            <c:if test="${not empty buyListItems}">
                                <a class="btn btn-approve" href="${checkoutUrl}">Checkout đơn mua</a>
                            </c:if>
                        </div>
                    </div>

                    <c:if test="${blockedBuyCount gt 0}">
                        <div class="student-inline-alert warn">Một số mục chưa thể gửi duyệt do hết hàng hoặc chưa có giá. Kiểm tra cột trạng thái trước khi checkout.</div>
                    </c:if>
                        ...
                        <table>
                            ...
                            <tbody>
                                <c:forEach var="item" items="${buyListItems}">
                                    <tr>
                                        ...
                                        <td>
                                            <c:choose>
                                                <c:when test="${item.canOrder}">
                                                    <span class="status-ok">Sẵn gửi duyệt</span>
                                                </c:when>
                                                <c:otherwise>
                                                    <span class="status-bad">Cần kiểm tra</span>
                                                </c:otherwise>
                                            </c:choose>
                                        </td>
                                        <td>
                                            ...
                                            <c:if test="${not item.canOrder}">
                                                <div class="note">Không thể gửi vì sách hết hàng hoặc chưa có giá.</div>
                                            </c:if>
                                        </td>
                                    </tr>
                                </c:forEach>
                                <c:if test="${empty buyListItems}">
                                    <tr>
                                        <td colspan="8" class="empty">Danh sách cần mua đang trống.</td>
                                    </tr>
                                </c:if>
                            </tbody>
                        </table>
                    ...
                    <c:if test="${not empty buyListItems}">
                        <div class="summary-note">Tổng tạm tính hiện tại là <strong>${buyListTotal}</strong>. Checkout sẽ gửi toàn bộ mục hợp lệ để thư viện xét duyệt.</div>
                    </c:if>
                </section>
                ...
                <div class="panel-stack">
                    <section class="card table-card" id="purchase-panel">
                        ...
                        <form method="get" action="${pageContext.request.contextPath}/borrows" class="search-form">
                            ...
                            <input type="hidden" name="bookPage" value="${bookCurrentPage}">
                            <div class="search-row student-search-row-single">
                                <input type="text" name="purchaseSearch" value="${purchaseSearch}" placeholder="Tìm theo mã đơn hoặc tên sách đã mua">
                                <div class="search-actions">
                                    <button class="btn-apply" type="submit">Tìm đơn đã duyệt</button>
                                    <a class="btn-reset" href="${pageContext.request.contextPath}${resetPurchaseFilterUrl}">Xóa lọc</a>
                                </div>
                            </div>
                        </form>
                            ...
                            <table class="compact-table">
                                ...
                                <tbody>
                                    ...
                                    <c:if test="${empty purchasedOrders}">
                                        <tr>
                                            <td colspan="4" class="empty">Bạn chưa có đơn mua nào đã duyệt.</td>
                                        </tr>
                                    </c:if>
                                </tbody>
                            </table>
                    ...
                    <section class="card table-card" id="borrow-panel">
                        ...
                        <c:if test="${returnRequestedCount gt 0}">
                            <div class="student-inline-alert">Có ${returnRequestedCount} phiếu đang chờ thư viện xác nhận trả. Bạn không cần gửi lại yêu cầu cho các phiếu này.</div>
                        </c:if>
                        ...
                        <c:if test="${activeBorrowCount gt 0 and overdueCount eq 0 and renewableBorrowCount eq 0 and returnRequestedCount eq 0}">
                            ...
                            <table class="compact-table">
                                ...
                                <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="borrow-action-stack">
                                                    ...
                                                    <c:if test="${b.status eq 'ReturnRequested'}">
                                                        <div class="borrow-action-note muted">Yêu cầu trả đã được gửi. Vui lòng chờ thư viện xác nhận.</div>
                                                    </c:if>
                                    ...
                                    <c:if test="${empty borrows}">
                                        <tr>
                                            <td colspan="7" class="empty">Bạn chưa có phiếu mượn nào.</td>
                                        </tr>
                                    </c:if>
                                </tbody>
                            </table>
